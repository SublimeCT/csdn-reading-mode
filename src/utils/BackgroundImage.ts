...
export interface BackgroundImageHandler {
}
...
export class BackgroundImage {
  ...
  static readonly handler: Record<BackgroundImageType, BackgroundImageHandler> = {
    baidu: {
    },
    file: {
    },
  ...
  get type(): BackgroundImageType {
    if (/^\d{1,3}$/.test(this.id)) {
      return BackgroundImageType.baidu
    } else if (this.id.indexOf('url-') === 0) {
      return BackgroundImageType.url
    } else if (this.id.indexOf('copy-') === 0) {
      return BackgroundImageType.copy
    } else {
      return BackgroundImageType.file
    }
  }

  /** 获取对应的 css 中的 background-image 值 */
  async getBackgroundImageValue(cssWrap: boolean = true) {
    if (this.type === BackgroundImageType.baidu) {
      return BackgroundImage._toBaiduUrl(Number(this.id), cssWrap)
    } else if (this.type === BackgroundImageType.url) {
      const url = await CustomBackgroundImage.getUrlByImageID(this.id)
      return cssWrap ? `url(${url})` : url
    } else {
      const url = await CustomBackgroundImage.getImageUrlByID(this.id)
      return cssWrap ? `url(${url})` : url
    }
  }
  ...
  /** 当前背景图 */
  static currentImage: BackgroundImage = new BackgroundImage('')
  ...
  static async getImage() {
    ...
    return BackgroundImage.currentImage
  }
